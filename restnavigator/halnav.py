...
class HALNavigator(object):
    ...
    def init_from_hal_json(cls, root_uri, hal_response):
        ...
        _halnavigator.populate_hal_properties(hal_json)
    ...
    def populate_hal_properties(self, body):
    ...
    def fetch(self, raise_exc=True, strict=True):
        ...
        self.populate_hal_properties(body)
    ...
    def create_navigator_or_orphan_resource(self, response):
        if response.status_code in (httplib.CREATED, # Applicable for POST
            ...
            return NonIdempotentResponse(parent=self, response=response)
        else:
            '''
                Expected hits:
